...
class Board:
    ...
    def reverseMove(self, piece, oldRow, oldCol, isKing, capturePiece = None):
        if abs(piece.row - oldRow) == 2:
            self.add(capturePiece)
        self.board[piece.row][piece.col] = 0
...
class Game2P:
    ...
    def winingWindow(self, winner=None):
        """
    ...
    def search(self, depth, turn, jumpMove, maxScore, minScore, lastPiece = None):
        if depth == 0:
            return self.board.red_left - self.board.blue_left
        if turn == red:
                ...
                for i in range(rows):
                    for j in range(cols):
                        ...
                        if piece != 0 and piece.color == turn:
                            for move in piece.posibleMove(self.board, jumpMove):
                                if len(move) == 3:
                                    captureMoves.append([piece, move])
                                else:
                                    normalMoves.append([piece, move])
            else:
                for move in lastPiece.posibleMove(self.board, jumpMove):
                    if len(move) == 3:
                        captureMoves.append([lastPiece, move])
                    else:
                        normalMoves.append([lastPiece, move])
            possibleMoves = captureMoves + normalMoves
            if len(possibleMoves) == 0:
                if jumpMove:
                    return self.search(depth - 1, blue, False, maxScore, minScore)
                return -100
            currentScore = -100
            for move in possibleMoves:
                ...
                if turn == red:
                    ...
                    if len(move[1]) == 2:
                        ...
                        nextTurn = blue
                if nextTurn == turn:
                    score = self.search(depth, nextTurn, True, maxScore, minScore, piece)
                else:
                    score = self.search(depth - 1, nextTurn, False, maxScore, minScore)
                self.board.reverseMove(piece, oldRow, oldCol, isKing, capturePiece)
            ...
            if len(possibleMoves) == 0:
                if jumpMove:
                    return self.search(depth - 1, red, False, maxScore, minScore)
                return 100
            currentScore = 100
            for move in possibleMoves:
                piece = move[0]
                ...
                if nextTurn == turn:
                    ...
                    score = self.search(depth - 1, nextTurn, False, maxScore, minScore)
                self.board.reverseMove(piece, oldRow, oldCol, isKing, capturePiece)
                currentScore = min(currentScore, score)
    ...
    def minimax(self, depth, turn, jumpMove, lastPiece = None):
        time = datetime.now()
        normalMoves, captureMoves = [], []
        if not jumpMove:
            for i in range(rows):
        ...
        for move in possibleMoves:
            ...
            if nextTurn == turn:
                ...
                score = self.search(depth - 1, nextTurn, False, maxScore, minScore)
            self.board.reverseMove(piece, oldRow, oldCol, wasking, capturePiece)
            if score > maxScore:
                bestMove = move
                maxScore = score
        self.board.move(bestMove[0], bestMove[1][0], bestMove[1][1])
        if len(bestMove[1]) == 3:
            self.minimax(depth, turn, True, bestMove[0])
        else:
            self.changeTurn()
        print('Thinking time: {}'.format(datetime.now() - time))
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                if piece != 0 and piece.color == game.turn:
                    if len(piece.posibleMove(game.board, False)) != 0:
                        out = False
                        break
        if out:
            game.changeTurn()
            game.winingWindow(game.turn)
            break
        if game.turn == red:
            out = game.minimax(8, game.turn, False)
            if out:
                return
        else:
            for event in pygame.event.get():
